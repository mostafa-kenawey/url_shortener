...
created_links = Enum.map(links, fn link_attrs ->
    ...
    {:error, changeset} -> 
      ...
      case UrlShortener.Links.get_link_by_slug(link_attrs.slug) do
        nil -> 
          ...
          nil
        existing_link ->
          IO.puts("ℹ️  Link already exists: #{existing_link.slug}")
          existing_link
      end
  end
end) |> Enum.filter(& &1)
...
ip_addresses = [
  ...
  "10.0.0.1", "10.0.0.2", "172.16.0.1", "172.16.0.2",
...
Enum.each(created_links, fn link ->
  ...
  click_count = case link.slug do
    "elixir" -> Enum.random(50..100)  # Most popular
    "phoenix" -> Enum.random(30..60)  # Second most popular  
    "docs" -> Enum.random(20..40)     # Moderate
    _ -> Enum.random(5..20)           # Less popular
  end
  ...
  Enum.each(1..click_count, fn _ ->
    ...
    metric_attrs = %{
      ...
      inserted_at: NaiveDateTime.from_iso8601!(DateTime.to_iso8601(click_time) |> String.replace("Z", ""))
    }
    
    # Insert directly to control timestamp
    changeset = UrlShortener.RedirectMetrics.RedirectMetric.changeset(
