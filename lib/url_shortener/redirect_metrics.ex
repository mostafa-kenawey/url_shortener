  ...
  def get_recent_clicks(limit \\ 10) do
    ...
    case Cache.get_analytics(cache_key) do
      ...
      {:error, :not_found} ->
        ...
        data = Repo.all(query)
        Cache.put_analytics(cache_key, data, :timer.minutes(2))  # Shorter cache for recent data
        data
    end
  end
  ...
  def get_clicks_over_time(days \\ 7) do
    ...
    case Cache.get_analytics(cache_key) do
      ...
      {:error, :not_found} ->
        ...
        data = Repo.all(query)
        Cache.put_analytics(cache_key, data, :timer.hours(1))  # Longer cache for historical data
        data
    end
  end
