defmodule UrlShortenerWeb.AdminConfirmationInstructionsLiveTest do
  use UrlShortenerWeb.ConnCase, async: true

  import Phoenix.LiveViewTest
  import UrlShortener.AdminFixtures

  alias UrlShortener.Admin.{Account, AdminToken}
  ...
  describe "Resend confirmation" do
    ...
    test "sends a new confirmation token", %{conn: conn, admin: admin} do
      ...
      assert Repo.get_by!(AdminToken, admin_id: admin.id).context == "confirm"
    end

    test "does not send confirmation token if admin is confirmed", %{conn: conn, admin: admin} do
      Repo.update!(Account.confirm_changeset(admin))
